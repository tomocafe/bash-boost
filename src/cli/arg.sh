    ...
    while [[ $# -gt 0 ]]; do
        case "$1" in
            ...
            --*)
                ...
                local optval="${optstr#*=}"
                [[ ${__bb_cli_arg_optvals["$longname"]+set} ]] || bb_cli_arg_errusage "invalid option: --$longname"
                if bb_cli_arg_isflag "$longname"; then
                    # Flag, e.g., --foo
                    ...
                    if [[ "$optstr" == "$longname" ]]; then
                        ...
                        shift
                        [[ $# -gt 0 ]] || bb_cli_arg_errusage "option --$longname missing required value"
                        optval="$1"
                    fi
            ...
            -*)
                ...
                for (( i=0; i<${#optstr}; i++ )); do
                    ...
                    [[ -n "$longname" ]] || bb_cli_arg_errusage "invalid option: -$shortname"
                    if bb_cli_arg_isflag "$longname"; then
                        # Flag, e.g., -f
                        __bb_cli_arg_optvals["$longname"]=$__bb_true
                    else
                        ...
                        if [[ ${#optstr} -eq 1 ]]; then
                            shift
                            [[ $# -gt 0 ]] || bb_cli_arg_errusage "option -$shortname missing required value"
                            __bb_cli_arg_optvals["$longname"]="$1"
                        else
                            bb_cli_arg_errusage "option -$shortname requires argument but is compounded as -$optstr; use -$shortname ${longname^^} -${optstr/$shortname/}"
                        fi
                    fi
                done
                ;;
